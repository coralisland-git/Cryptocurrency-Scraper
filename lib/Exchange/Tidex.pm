...
sub _init {
  ...
  $self->{config} = {
      'id' => 'tidex',
            ...
            'urls' => {
                'logo' => 'https://tidex.com/img/logo-white.svg',
                'api' => 'https://api.tidex.com/api/',
                'www' => 'https://tidex.com/',
...
sub ticker {
  ...
  foreach my $symbol (@{$self->{config}->{symbols}}) {
    ...
    $self->get(
      url => "https://api.tidex.com/api/3/ticker/$symbol->{id}",
...
sub _get_symbols {
  my $self = shift;
  my $result = $self->ua->get('https://api.tidex.com/api/3/info')->result->json->{pairs};
  my $new_result;
